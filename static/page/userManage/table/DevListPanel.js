export class DevListPanel {
    ...
    static drawUserPhone(data) {
        ...
        doOperationCheckBoxTh2 = `
            <td class="am-text-middle table-td-selector" style="width:45px;">
                <div class="myCheckBox">
                    <input class="all-checkbox-phone" type="checkbox">
                </div>
            </td>
        `;
        ...
        for(let i=0;i<data.length;i++){
            ...
            doOperationCheckBoxTd2 = `
                    <td class="am-text-middle table-td-selector">
                        <div class="myCheckBox">
                            <input class="checkbox-phone"  type="checkbox" data-phone-id="${data[i].id}">
                        </div>
                    </td>
                `;
            htmlValue2 = htmlValue2 + `
                <tr class="tbody-tr" style="border-bottom:1px solid rgb(100,100,100);">
                    ${doOperationCheckBoxTd2}
                    <td data-id="${data[i].id}" class="discernment-dev">${data[i].phoneNum}</td>
                    <td>${startTime(data[i].startTime)}</td>
                    <td>${endTime(data[i].endTime)}</td>
                    
                    <td class="table-in-todo">
                        
                        <button type="button" class="table-in-btn btn-blue update-phone" data-phoneId="${data[i].id}" data-index="${i}">修改</button>
                        <button type="button" class="table-in-btn btn-red delete-phone" data-index="${i}" data-phone-id="${data[i].id}">删除</button>
                    </td>
                </tr>

            `;
        }
        $('#phone-table-tbody').append(htmlValue2);
    };
    //获取机构下的测试手机号
