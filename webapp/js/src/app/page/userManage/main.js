...
$(document).on('click', '.update-user', function () {
    ...
    UserOperation.operation(null, `getUserDetailInfo/${userInfo.id}`, 'GET').then((result) => {
        ...
        if (result.retcode == 1 && result.dataMap != null) {
            ...
            if(result.dataMap.user.devPerms){
                if(result.dataMap.user.devPerms.devUseType =='1'){
                    $('#set-devPermsStartTime').css('display','none');
                    
                    $('#devUseType').html('本人下属设备');
                }
                if(result.dataMap.user.devPerms.devUseType =='2'){
                    $('#set-devPermsStartTime').css('display','');
                    
                    $('#devUseType').html('本人及所在单位设备');
                }
                if(result.dataMap.user.devPerms.devUseType =='3'){
                    $('#set-devPermsStartTime').css('display','');
                    
                    $('#devUseType').html('本人及所在单位及下属单位设备');
                }
    
                if(result.dataMap.user.devPerms.devUseType =='4'){
                    $('#set-devPermsStartTime').css('display','');
                   
                    $('#devUseType').html('系统内全部设备');
                }
                if(result.dataMap.user.devPerms.devUseType =='5'){
                    $('#set-devPermsStartTime').css('display','');
                    
                    $('#devUseType').html('不限');
                }
                if(result.dataMap.user.devPerms.phoneUseType =='1'){
                    $('#set-phonePermsStartTime').css('display','none');
                    
                    $('#phoneUseType').html('本人下属手机号');
                }
                if(result.dataMap.user.devPerms.phoneUseType =='2'){
                    $('#set-phonePermsStartTime').css('display','');
                    
                    $('#phoneUseType').html('本人及所在单位手机号');
                }
                if(result.dataMap.user.devPerms.phoneUseType =='3'){
                    $('#set-phonePermsStartTime').css('display','');
                    
                    $('#phoneUseType').html('本人及所在单位及下属单位手机号');
                }
    
                if(result.dataMap.user.devPerms.phoneUseType =='4'){
                    $('#set-phonePermsStartTime').css('display','');
                    
                    $('#phoneUseType').html('系统内全部手机号');
                }
                if(result.dataMap.user.devPerms.phoneUseType =='5'){
                    $('#set-phonePermsStartTime').css('display','');
                    
                    $('#phoneUseType').html('不限');
                }
                $('#query-time-start-timeEig').val(result.dataMap.user.devPerms.devPermsStartTime==null? '' : new Date(result.dataMap.user.devPerms.devPermsStartTime).Format('yyyy-MM-dd hh:mm:ss'));
                $('#query-time-end-timeEig').val(result.dataMap.user.devPerms.devPermsEndTime==null? '' : new Date(result.dataMap.user.devPerms.devPermsEndTime).Format('yyyy-MM-dd hh:mm:ss'));
                $('#query-time-start-timeNin').val(result.dataMap.user.devPerms.phonePermsStartTime==null? '' : new Date(result.dataMap.user.devPerms.phonePermsStartTime).Format('yyyy-MM-dd hh:mm:ss'));
                $('#query-time-end-timeNin').val(result.dataMap.user.devPerms.phonePermsEndTime==null? '' : new Date(result.dataMap.user.devPerms.phonePermsEndTime).Format('yyyy-MM-dd hh:mm:ss'));
            }else{
                $('#set-devPermsStartTime').css('display','none');
...
/*
$('#dept-selector-button').click(function () {
    if ($('#dept-query-selector-container').css('display') == 'none') {
        $('#dept-query-selector-container').css('display', 'block');
    } else {
        $('#dept-query-selector-container').css('display', 'none');
    }
});*/
/* add by guoshuai start 2018-8-24*/
...
/* add by guoshuai start 2018-8-31*/
//移入移出效果  
// $("#div_items div").hover(function () {  
//     $("#div_items div").css('background-color', '#1C86EE').css('color', 'white');  
// }, function () {  
//     $("#div_items div").css('background-color', 'white').css('color', 'black');  
// }); 
$("body").click(function () {  
